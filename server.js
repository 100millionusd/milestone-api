...
if (willUseSafe) {
  try {
    ...
    // 5) prepare EIP-712 typed data (SafeTx)
    ...
    const domain = {
      chainId,
      verifyingContract: SAFE_ADDRESS_CS
    };
    ...
    // 6) sign typed data (EIP-712) and SELF-VERIFY before posting
    ...
    const body = {
      ...
      refundReceiver: ZERO,
      nonce,
      // NOTE: with EIP-712 the service recomputes the hash from the fields above,
      // so we don't need to send contractTransactionHash; but it's allowed to include it:
      // contractTransactionHash: ethers.utils._TypedDataEncoder.hash(domain, types, value),
      sender: senderAddr,
    ...
    // 8) store a deterministic tx hash for tracking (hash the typed data)
    const safeTxHash = ethers.utils._TypedDataEncoder.hash(domain, types, value);
    await pool.query(
      `UPDATE milestone_payments
