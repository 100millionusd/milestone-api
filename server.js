...
app.get("/api/proofs/change-requests", authRequired, async (req, res) => {
  try {
    ...
    // Use quoted identifiers to match Prisma's default case-sensitive naming
    let query = `
      SELECT cr."id", cr."proposalId", cr."milestoneIndex", cr."status", cr."comment", cr."checklist", cr."createdAt", cr."resolvedAt",
             (SELECT jsonb_agg(jsonb_build_object(
                'id', resp."id",
                'createdAt', resp."createdAt",
                'note', resp."comment",
                'files', resp."filesJson"
              ) ORDER BY resp."createdAt" ASC)
              FROM "ProofChangeResponse" resp
              WHERE resp."requestId" = cr."id") as responses
      FROM "ProofChangeRequest" cr
      WHERE 1=1
    `;
    // NOTE: Prisma schema does not have tenant_id on ProofChangeRequest. 
    // If the main app doesn't use tenant_id for CRs, we shouldn't filter by it, 
    // OR we should check if the proposal belongs to the tenant.
    // For now, to fix the "missing requests" issue, we remove the tenant_id filter 
    // because the table doesn't have it!

    const params = [];
    let idx = 1;

    if (proposalId) {
      query += ` AND cr."proposalId" = $${idx++}`;
      params.push(proposalId);
    }
    if (milestoneIndex !== undefined && milestoneIndex !== null && milestoneIndex !== '') {
      query += ` AND cr."milestoneIndex" = $${idx++}`;
...
app.post("/api/proofs/change-requests", adminGuard, async (req, res) => {
  ...
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO "ProofChangeRequest" 
       ("proposalId", "milestoneIndex", "comment", "checklist", "status")
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [proposalId, milestoneIndex, comment, checklist || [], status || 'open']
    );
...
app.post("/api/proofs/change-requests/:id/respond", authRequired, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO "ProofChangeResponse"
       ("requestId", "comment", "filesJson", "createdBy")
       VALUES ($1, $2, $3, 'vendor')
       RETURNING *`,
      [crId, comment, JSON.stringify(files || [])]
    );
