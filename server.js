...
// Approve a single proof by its proof_id (admin only) — keeps both admin views in sync
app.post('/proofs/:id/approve', adminGuard, async (req, res) => {
  const proofId = Number(req.params.id);
  ...
  try {
    ...
    // 2) Flip THIS proof to approved (idempotent)
    let approved = proof;
    if (String(proof.status || '').toLowerCase() !== 'approved') {
      ...
      approved = upd[0];
      // audit trail on the bid
      try { await writeAudit(approved.bid_id, req, { proof_approved: { index: approved.milestone_index, proofId } }); } catch {}
    }

    // 3) Mark the milestone completed in the bid JSON (keeps project overview consistent)
    const { rows: br } = await pool.query('SELECT * FROM bids WHERE bid_id=$1', [ approved.bid_id ]);
    const bid = br[0];
    if (bid) {
      const milestones = Array.isArray(bid.milestones) ? bid.milestones : JSON.parse(bid.milestones || '[]');
      const idx = Number(approved.milestone_index);
      if (Number.isInteger(idx) && milestones[idx]) {
        const ms = milestones[idx];
        ms.completed = true;
        if (!ms.completionDate) ms.completionDate = new Date().toISOString();
        if (note && !ms.proof) ms.proof = note; // optional: store an admin note if none exists
        await pool.query('UPDATE bids SET milestones=$1 WHERE bid_id=$2', [ JSON.stringify(milestones), approved.bid_id ]);
      }

      // 4) Notify (matches your existing complete‑milestone behavior)
      if (NOTIFY_ENABLED) {
        try {
          const { rows: [proposal] } = await pool.query('SELECT * FROM proposals WHERE proposal_id=$1', [ bid.proposal_id ]);
          if (proposal && typeof notifyProofApproved === 'function') {
            await notifyProofApproved({
              ...
              bid,
              proposal,
              msIndex: Number(approved.milestone_index) + 1,
            });
          }
        } catch (e) {
          console.warn('notifyProofApproved /proofs/:id/approve failed:', String(e).slice(0, 200));
        }
      }
    }

    // 5) Return the updated proof in camelCase (consistent with your other /proofs responses)
    return res.json(toCamel(approved));
  } catch (e) {
    console.error('POST /proofs/:id/approve failed', e);
    return res.status(500).json({ error: 'Internal error' });
  }
